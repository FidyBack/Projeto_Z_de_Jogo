...
class Jogo:
    ...
    def eventos(self):
        ...
        for evento in pg.event.get():
            ...
            if evento.type == pg.KEYDOWN:
                if (
                    evento.key == pg.K_SPACE
                    or evento.key == pg.K_w
                    or evento.key == pg.K_UP
                ):
                    self.jogador.pulo()
        ...
        if self.jogador.tiro_reto and self.jogador.contador_tiro >= 6:
            Tiro_reto(
                self,
                self.jogador.posi + self.jogador.posicao_arma[:],
                self.jogador.vel_tiro_reto[:],
                self.jogador.velo[:],
                self.jogador.olhar_direita,
            )
        ...
        if (
            self.jogador.tiro_parabola
            and self.jogador.contador_tiro >= 6
            and self.jogador.numero_granada > 0
        ):
            Tiro_parabola(
                self,
                self.jogador.posi + self.jogador.posicao_arma[:],
                self.jogador.vel_tiro_parabola[:],
                self.jogador.velo[:],
                self.jogador.olhar_direita,
            )
            self.jogador.contador_tiro = 0
    ...
    def update(self):
            ...
            if impacto:
                for plataforma in impacto:
                    ...
                    if (
                        ...
                        and personagem.rect.right > plataforma.rect.left
                        and abs(intersect.width) >= abs(intersect.height)
                    ):
                        if (
                            personagem.velo.y > 0
                            and personagem.rect.bottom == intersect.bottom
                        ):
                            ...
                            personagem.velo.y < 0
                            and personagem.rect.top == intersect.top
                        ):
                    ...
                    if (
                        personagem.rect.top < plataforma.rect.bottom
                        and personagem.rect.bottom > plataforma.rect.top
                        and abs(intersect.height) > abs(intersect.width)
                    ):
                        if (
                            personagem.velo.x > 0
                            and personagem.rect.right == intersect.right
                        ):
                            ...
                            personagem.velo.x < 0
                            and personagem.rect.left == intersect.left
                        ):
                            personagem.rect.left = plataforma.rect.right
                            personagem.posi = vec(personagem.rect.midbottom)
                            personagem.velo.x = 0
        ...
        colisao_mob = pg.sprite.spritecollide(
            self.jogador, self.inimigos, False, pg.sprite.collide_mask
        )
        if colisao_mob:
        ...
        for inimigo in self.inimigos:
            tiro_para_inimigo = pg.sprite.spritecollide(
                inimigo, self.tiro_personagem, False, pg.sprite.collide_mask
            )
        ...
        for powerup in self.powerup:
            colisao_powerup_plataforma = pg.sprite.spritecollide(
                powerup, self.plataforma, False
            )
    ...
    def mostrar_tela_inicio(self, imagem, texto):
        ...
        b = 0
        brilho = 0
        image = pg.image.load(path.join(path.dirname(__file__), imagem))
        ...
        while i < 7:
            ...
            self.desenhar_texto(
                texto, 20, (brilho, brilho, brilho), largura / 2, altura * 7 / 8
            )
            self.desenhar_texto(
                "Shoot : j  grenade : i  Move: w/a/s/d",
                20,
                branco,
                largura / 2,
                altura * 3 / 4,
    ...
    def mostrar_tela_final(self, imagem, texto):
        ...
        while i < 7:
            ...
            self.desenhar_texto(
                "Tente Novamente", 20, branco, largura / 2, altura * 3 / 4
            )
            pg.display.flip()
    ...
    def load_data(self):
        ...
        self.spritesheet_inimigos = Spritesheet(
            path.join(img_dir, spritesheet_inimigos)
        )
        self.spritesheet_personagem = Spritesheet(
            path.join(img_dir, spritesheet_personagem)
        )
        self.spritesheet_plataformas = Spritesheet(
            path.join(img_dir, spritesheet_plataformas)
        )
    ...
    def desenho(self):
        ...
        self.desenhar_texto(
            f"Vida = {self.jogador.vida}, Granada = {self.jogador.numero_granada}",
            20,
            branco,
            120,
            0,
    ...
    def musica(self, musica, repeticoes):
        pg.mixer.music.load(path.join(path.dirname(__file__), musica))
